...
def getMask(X, alpha):
	...
	mask[(geneAvgs[None,:] > alpha) & (X == 0)] = 0
...
data_suffix = 'dropout_index_5_seed_20000'
out_suffix = '5_20000'
...
data_full = data_full.to_numpy()
#data = data.loc[1:10,1:6].T
#print(data.to_numpy)
data = data.T.to_numpy()#[:1000,:]
print("Data with ", data.shape[0], " cells, and ", data.shape[1], " genes")
#print(data[:5, :5])

maxZerosInCell = 101*data.shape[1]/100
maxZerosInGene = 101*data.shape[0]/100
small_zero_thre = 0.6#0.6#0.7
data_f, filteredGenes, filteredCells, maxPerCell = preprocessData(data, maxZerosInCell, maxZerosInGene)
...
mask, geneAvgs = getMask(data_f, small_zero_thre)
...
Z = tf.placeholder(tf.float32, shape = [None, Dim], name="Z")
NZ = tf.placeholder(tf.float32, shape = [None, Dim], name="NZ")

#%% 2. Discriminator
D_W1 = tf.Variable(xavier_init([Dim*2, 256]), name="D_W1")     # Data + Hint as inputs
D_b1 = tf.Variable(tf.zeros(shape = [256]), name="D_b1")

D_W2 = tf.Variable(xavier_init([256, 128]), name="D_W2")
D_b2 = tf.Variable(tf.zeros(shape = [128]), name="D_b2")

D_W3 = tf.Variable(xavier_init([128, Dim]), name="D_W3")
D_b3 = tf.Variable(tf.zeros(shape = [Dim]), name="D_b3")       # Output is multi-variate

theta_D = [D_W1, D_W2, D_W3, D_b1, D_b2, D_b3]

#%% 3. Generator
G_W1 = tf.Variable(xavier_init([Dim*2, 256]), name="G_W1")     # Data + Mask as inputs (Random Noises are in Missing Components)
G_b1 = tf.Variable(tf.zeros(shape = [256]), name="G_b1")

EMB_SIZE=128
G_W2 = tf.Variable(xavier_init([256, EMB_SIZE]), name="G_W2")
G_b2 = tf.Variable(tf.zeros(shape = [EMB_SIZE]), name="G_b2")

G_W3 = tf.Variable(xavier_init([EMB_SIZE, Dim]), name="G_W3")
...
def generator(x,z,m):
    ...
    inputs = tf.concat(axis = 1, values = [inp,m], name="G_input_cat")  # Mask + Data Concatenate
    G_h1 = tf.nn.relu(tf.matmul(inputs, G_W1) + G_b1, name="G_h1")
    G_h2 = tf.nn.relu(tf.matmul(G_h1, G_W2) + G_b2, name="G_h2")
    G_prob = tf.nn.sigmoid(tf.matmul(G_h2, G_W3) + G_b3) # [0,1] normalized Output
    #G_prob = tf.nn.relu(tf.matmul(G_h2, G_W3) + G_b3, name="G_output") 
...
def discriminator(x, m, g, h):
    ...
    inputs = tf.concat(axis = 1, values = [inp,h], name="D_input_cat")  # Hint + Data Concatenate
    D_h1 = tf.nn.relu(tf.matmul(inputs, D_W1) + D_b1, name="D_h1")
    D_h2 = tf.nn.relu(tf.matmul(D_h1, D_W2) + D_b2, name="D_h2")
    D_logit = tf.matmul(D_h2, D_W3) + D_b3
    D_prob = tf.nn.sigmoid(D_logit, name="D_output")  # [0,1] Probability Output
...
for it in tqdm(range(100020)):    
    ...
    _, D_loss_curr, D_bias3, D_bias1 = sess.run([D_solver, D_loss1, D_b3, D_b1], feed_dict = {X: X_mb, M: M_mb, Z: New_X_mb, H: H_mb, NZ:(X_mb > 0)*1})
    ...
    if it % 1000 == 0 and it > 1:
        ...
        print('D_b3: {:.4}'.format(np.mean(D_bias3)))
        print('D_b1: {:.4}'.format(np.mean(D_bias1)))
	...
	print("NZeros=", np.count_nonzero(1-testM)/(imp.shape[0]*imp.shape[1]*1.0), np.count_nonzero(imp*(1-np.transpose(testM)))*1.0/np.count_nonzero(1-testM))
        ...
        #np.savetxt('imputation_gain_data/'+'/gain_'+out_suffix+"_"+str(it)+'_transformed.csv', imp, delimiter=',', fmt="%f")
    ...
    if it == 1000:
        mask, geneAvgs = getMask(data_f, 0.4)
        trainM = mask
        testM = trainM[:,:]
        trainX = np.transpose(imp)
        testX = trainX[:,:]
        np.savetxt('imputation_gain_data/'+'/gain_'+out_suffix+"_"+str(it)+'_reproduced.csv', imp, delimiter=',', fmt="%f")
    if it == 3000:
        mask, geneAvgs = getMask(data_f, 0.25)
        trainM = mask
        testM = trainM[:,:]
        trainX = np.transpose(imp)
        testX = trainX[:,:]
        np.savetxt('imputation_gain_data/'+'/gain_'+out_suffix+"_"+str(it)+'_reproduced.csv', imp, delimiter=',', fmt="%f")
    if it == 6000:
        mask, geneAvgs = getMask(data_f, 0.2)
        trainM = mask
        testM = trainM[:,:]
        trainX = np.transpose(imp)
        testX = trainX[:,:]
